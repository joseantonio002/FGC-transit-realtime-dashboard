...
# 13/07/2026 Shape-based train marker animation

Goal: make train markers feel alive between GTFS-Realtime vehicle snapshots without drifting away from the rail tracks.

Problem with the first idea: moving markers by raw latitude/longitude deltas can look very bad. Some trains do not move because there is no stable previous marker identity, and trains that do move can leave the tracks because GPS points are noisy and a straight-line extrapolation does not know the rail geometry.

Difficulties:

- The frontend needs persistent marker state. If every refresh clears and recreates all markers, there is no previous position to animate from.
- The browser should not move trains freely across the map. It must constrain them to `shapes.json`.
- Existing vehicle JSON did not include `shape_id`, `trip_id`, `vehicle_id`, or timestamp, so the frontend could not reliably know which shape or train identity each marker belonged to.
- The GitHub Pages demo in `docs/` uses already collected static JSON frames, so those files do not have the new identifiers unless the demo data is regenerated.

Solution implemented:

- Added `trip_id`, `vehicle_id`, `shape_id`, and `timestamp` to the collector's vehicle JSON output. `shape_id` comes from joining GTFS-Realtime `trip_id` with GTFS Static `trips.txt`.
- Indexed every shape in the frontend by `shape_id` and `other_ids`.
- Converted train positions from raw coordinates into a distance along the selected shape by projecting the real point onto the closest shape segment.
- Kept train markers in a map keyed by `vehicle_id` or `trip_id`, instead of clearing all markers every refresh.
- Between data refreshes, markers advance a small distance along the same shape. When a new real point arrives, the marker corrects back to the newly projected position over a short animation window.
- For `docs/`, added a nearest-shape fallback so the current static demo frames still animate even though they do not contain `shape_id`. This is less reliable than exact `shape_id`, but it preserves GitHub Pages functionality until the demo frames are regenerated.

Expected tradeoff: with exact `shape_id`, trains should stay on tracks. There can still be occasional correction jumps when the feed updates, because the realtime positions are delayed/noisy and animation is predictive between snapshots. The important change is that those corrections happen along a rail shape instead of off-track.
